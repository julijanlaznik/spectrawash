...
const Vouchers: React.FC = () => {
  ...
  return (
    <section id="vouchers" className="py-16 md:py-24 bg-brand-light relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {VOUCHERS.map((voucher, index) => (
            <motion.div
              ...
              className="bg-white p-8 md:p-10 border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden"
            >
              {/* Top accent line */}
              ...
              <div className="flex justify-between items-start mb-6">
                <h3 className="text-xl md:text-2xl font-bold font-heading text-brand-dark group-hover:text-brand-blue transition-colors">
                    {voucher.title}
                </h3>
                <span className="bg-brand-light text-brand-blue font-bold px-3 py-1 text-sm tracking-wide whitespace-nowrap ml-4">
                    {voucher.price}
                </span>
              </div>
              ...
              <ul className="space-y-4 mb-10">
                {voucher.features.map((feature, i) => (
                    <li key={i} className="flex items-center text-sm text-brand-dark/80">
                        <span className="w-1.5 h-1.5 bg-brand-blue mr-3 rounded-full flex-shrink-0"></span>
                        {feature}
                    </li>
                ))}
              </ul>
